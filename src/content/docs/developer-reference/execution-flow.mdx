...
| `eugr` | Builds [eugr/spark-vllm-docker](https://github.com/eugr/spark-vllm-docker) containers. Clones the repo, runs `build-and-copy.sh` with recipe-specified build args, and converts recipe `mods` into `pre_exec` entries. | Recipes with `builder: eugr`, v1 recipes with `build_args`/`mods`, or recipes using `ghcr.io/spark-arena/dgx-vllm-eugr-nightly` images (auto-detected) |
...
- **`post_exec`** — runs after a successful detached launch (Phase 6), once the serve port is ready. Executes inside the head container.
