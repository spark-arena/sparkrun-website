...
:::note[Which host is the head?]
The head is the first host of the *scheduled placement*, not automatically the
first host in your cluster definition. With the default `occupancy-sparse`
[scheduler](/advanced/schedulers/), placement is chosen from the least-loaded
hosts, so the head can differ between launches as the cluster fills up. Run
`sparkrun status` to see which host actually took the head role — its output
also gives you the ready-to-use `logs` and `stop` commands.
:::
...
- **Node selection**: If your cluster has more hosts than the requested TP level, sparkrun uses only as many as the job needs. Which ones depends on the [scheduler](/advanced/schedulers/): the default `occupancy-sparse` picks the least-loaded hosts, while `greedy` always takes the first N in the definition.
