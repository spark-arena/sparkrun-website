...
If you're using a cluster, pass it to the proxy so it can discover endpoints on
those hosts:
...
:::caution[Why `--host 127.0.0.1`]
Started with no bind host ever configured, the proxy keeps a legacy `0.0.0.0`
default and warns loudly: that exposes every discovered inference backend to
your entire network, unauthenticated unless you also set a master key. Choosing
a bind address explicitly silences the warning and is persisted to
`proxy.yaml`, so you only pass it once.

To share the endpoint deliberately, set `--master-key <secret>` as well.
:::
