...
sparkrun benchmark qwen3-1.7b-sglang --profile spark-arena-v1 --solo

# Re-run the same command — sparkrun skips completed tasks and resumes
sparkrun benchmark qwen3-1.7b-sglang --profile spark-arena-v1 --solo
```
...
To keep results comparable across long, possibly-resumed sweeps, sparkrun **pins the benchmarking framework version** (e.g. the `llama-benchy` release) on the first task and reuses that exact version for every subsequent task in the same run — even if a newer release ships mid-sweep. The pinned version is recorded in the output metadata.

State files live under `~/.cache/sparkrun/benchmarks/`. See the [benchmark CLI reference](/cli/benchmark/#scheduling-and-resumability) for the full details.
