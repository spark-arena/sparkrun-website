...
4. **CX7 configuration** — detects ConnectX-7 interfaces, selects conflict-free subnets, assigns static IPs with jumbo frames (MTU 9000), and distributes host keys. If CX7 IPs are added or changed, the SSH mesh is automatically re-run to ensure full connectivity across all interfaces.
5. **Docker group** — ensures the SSH user is in the `docker` group on all hosts so containers can run without sudo
6. **Sudoers entries** — installs scoped sudoers rules for `fix-permissions` and `clear-cache` (no broad sudo)
7. **earlyoom** — installs and configures OOM protection with inference-aware defaults
