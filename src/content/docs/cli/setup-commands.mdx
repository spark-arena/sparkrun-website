...
The SSH access phase is written to run on a bare Windows control node: it needs
only `ssh` and `ssh-keygen` (both shipped with Windows 10+), never a local
`bash`. Key material travels over stdin rather than the command line so it
can't be mangled by local shell quoting.
:::
...
Probes each host for the things the wizard configures and reports gaps —
**without changing anything**. Use it to verify a cluster before a first run,
or to find what drifted after one.
...
[`sparkrun setup rdma-test`](#rdma-performance-test).
...
Manage the gates on experimental capabilities. See
[Feature Flags](/advanced/feature-flags/) for the full list and resolution
order.
...
See [Updates & Telemetry](/getting-started/updates/#anonymous-telemetry) for
what is collected.
...
| `--diagnose` | Run SSH diagnostics instead of mesh setup (see [SSH Setup — Cross-user SSH](/getting-started/ssh-setup/#cross-user-ssh-control-machine--cluster-user)) |
...
Measures what `setup cx7` configured: RDMA latency and bandwidth per link, and optionally an NCCL
...
:::caution
Experimental — gated behind the `cli.setup.rdma_test` feature flag:

```bash
sparkrun setup features enable cli.setup.rdma_test
```

Unlike `setup check`, this command sends real traffic and takes a few minutes.
:::
...
Each host pair is measured link by link and then with **all its links driven concurrently** — on DGX
Spark a single QSFP112 cable presents as two RDMA devices, so a per-device figure is about half the
...
is a failure and exits `1`. See [Networking Best Practices](/getting-started/networking/) for the
numbers to expect.
