...
- `sparkrun setup check` reports **FAIL** on `NVIDIA CDI spec (/etc/cdi/nvidia.yaml)` with `/etc/cdi/nvidia.yaml missing or empty`
...
The spec records driver library and device paths as they were when it was generated. After an NVIDIA driver or firmware update those paths can move, leaving a stale spec that resolves to nothing. Re-run the same command to refresh it.
:::
