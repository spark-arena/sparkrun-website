...
### Sentinel images for managed builds

The two Spark Arena nightly image names — `ghcr.io/spark-arena/dgx-vllm-eugr-nightly:latest` and `ghcr.io/spark-arena/dgx-vllm-eugr-nightly-tf5:latest` — are **sentinel images**. They resolve to real container images, but when you pair them with the `:latest` tag, sparkrun's `eugr` builder treats them as an explicit user opt-in to **managed rebuilds**: it will clone the upstream [eugr/spark-vllm-docker](https://github.com/eugr/spark-vllm-docker) repo and build a fresh image from the current state of that repo (and current available wheels) before each launch.

This is the **default for all official v2 recipes** because it gives users a stable, single name to reference while sparkrun handles versioning. To opt out:

- Use any **non-`:latest` tag** (e.g. `ghcr.io/spark-arena/dgx-vllm-eugr-nightly:2026-03-01`) — sparkrun will pull that immutable tag and never rebuild.
- Use a **non-sentinel image name** (any image not under `ghcr.io/spark-arena/dgx-vllm-eugr-nightly*`) — sparkrun will pull (or rely on the local copy) and never rebuild, even at `:latest`.

The sentinel design is intentional: using a local image often implies the user manages versioning outside sparkrun, so sparkrun never silently forces a rebuild of an arbitrary `:latest` image. Sentinels make the "rebuild from upstream" intent explicit.
