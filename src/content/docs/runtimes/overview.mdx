...
| `atlas` | [Atlas](https://atlasinference.io/) | Coming soon | Pure-Rust inference server with native NCCL distribution (currently `tp=1` only) |
...
- **Atlas** is a pure-Rust inference server (see [Atlas on GitHub](https://github.com/Avarok-Cybersecurity/atlas)) with native NCCL distribution. Currently restricted to single-node (`tensor_parallel: 1`); multi-node tensor and expert parallelism support is in development.
...
:::tip[Sentinel images and managed rebuilds]
The two `dgx-vllm-eugr-nightly*` names above are **sentinel images**: when used with the `:latest` tag, sparkrun's `eugr` builder treats them as an opt-in to **managed rebuilds** from the current state of [eugr/spark-vllm-docker](https://github.com/eugr/spark-vllm-docker). Any other tag or any non-sentinel image is pulled normally and never silently rebuilt — so users who manage versioning outside sparkrun keep full control. See [vLLM › Sentinel images](/runtimes/vllm/#sentinel-images-for-managed-builds).
:::
